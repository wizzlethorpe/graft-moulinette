...
- Recovering provenance after the fact means inference, not a lookup.
  Moulinette writes a document's dependencies to disk but never the document,
  so nothing local says which asset an import came from. The README covers what
  this costs an author who enabled the module too late.

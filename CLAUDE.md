...
Release graft first, then this: a reader who installs this against graft 0.6.x gets a module that will not enable.
...
- graft's hooks, all three of which this module registers on: `graftPreBuild` may fire collect-only, so registration must do nothing, and this module takes the `sources` phase so it runs after a vault has expanded its marker; a transform returns `{ entries, skipped, warnings }` and runs once per build. `graftExport` hands over a finished Copy graft entry to rename. `graftBuilt` carries the built UUIDs.
- graft must ship before this module: its manifest requires graft 0.7.0, the first with `graftPreBuild`.
